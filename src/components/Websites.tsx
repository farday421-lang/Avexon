...
export default function Websites({ onOrderRequest }: WebsitesProps) {
  const { websites, offerConfig } = useContent();
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-8 z-10 flex flex-col justify-between">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-semibold uppercase tracking-wider"
          >
          ...
          >
            ফুড ডেলিভারি অ্যাপের মতো সহজে নির্বাচন করুন। পছন্দসই ডেমো পরখ করুন এবং কোনো জটিল কোডিং ছাড়াই মাত্র ৩ দিনে সম্পূর্ণ বাংলায় এডমিন ড্যাশবোর্ড সহ সাইটটি বুঝে নিন।
          </motion.p>
        </div>
        ...
        >
          {websites.map((web, idx) => {
            const showDiscountPrice = offerConfig?.discountActive && offerConfig?.discountPercentage;
                      ...
                      <div className="flex items-baseline gap-0.5 sm:gap-1">
                        {showDiscountPrice ? (
                          ...
                          <>
                            ...
                            <span className="text-[9px] sm:text-[11px] text-slate-500 line-through font-medium font-sans">৳{web.originalPrice.toLocaleString("bn-BD")}</span>
                          </>
                        )}
                      </div>
                    </div>
                ...
                <div className="pt-3.5 border-t border-purple-500/20 flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#0c0418] z-10 relative">
                  <div className="text-left w-full sm:w-auto flex sm:flex-col justify-between items-center sm:items-start">
                    <span className="text-[10px] sm:text-[11px] text-slate-500 font-extrabold uppercase tracking-widest">
                      {offerConfig?.discountActive && offerConfig?.discountPercentage ? "স্পেশাল অফার প্রাইজ (এককালীন)" : "এককালীন ফাইনাল প্রাইস"}
                    </span>
                    <p className="text-lg sm:text-2xl font-black text-purple-400 font-sans mt-0.5">
                      {offerConfig?.discountActive && offerConfig?.discountPercentage ? (
                        <>
                    ...
                    <div className="md:hidden bg-[#0c041bcc]/95 backdrop-blur-md border-t border-purple-500/20 p-3 px-4 flex items-center justify-between gap-3 z-40 shrink-0 select-none">
                      ...
                      <div className="text-center">
                        <div className="text-[9px] text-slate-400 font-medium uppercase tracking-wider leading-none mb-0.5">
                          {offerConfig?.discountActive && offerConfig?.discountPercentage ? "অফার মূল্য" : "এককালীন মূল্য"}
                        </div>
                  ...
                  <div className="pt-4 border-t border-purple-500/10 space-y-3">
                    ...
                    <div className="flex justify-between items-center px-1">
                      <span className="text-[10px] text-slate-500 font-extrabold uppercase tracking-widest leading-none">
                        {offerConfig?.discountActive && offerConfig?.discountPercentage ? "অফার ডেলিভারি মূল্য" : "এককালীন ডেলিভারি মূল্য"}
                      </span>
...
function LiveSellAndOfferTimer() {
  ...
  useEffect(() => {
    if (!offerConfig || !offerConfig.show) return;
